...
spec =
  do
    describe "Serverless Validator" $ do
      ...
      it "validates framework version" $ do
        S.mkFrameworkVersion "1" `shouldBe` Left "Framework version must be a string of the form: >=x.x.x <x.x.x"
        ...
        S.mkFrameworkVersion ">=1.0.0 <2.0.0" `shouldBe` Right S.FV { S.frameworkVersionMin = Maybe.fromJust . S.toSemVer $ "1.0.0"
